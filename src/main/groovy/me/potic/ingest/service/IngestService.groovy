...
class IngestService {
    ...
    void ingestNewArticles() {
        ...
        userService.allUsersIds().forEach { String userId ->
            long ingestSince = userArticlesIngestedTill.getOrDefault(userId, 0)
            Collection<Article> ingestedArticles = pocketApiService.getArticlesByUserId(userId, ingestRequestSize, ingestSince)
            ...
            if (!ingestedArticles.empty) {
                ...
                userArticlesIngestedTill.put(userId, ingestedTill)
            }
        }
    }
}
