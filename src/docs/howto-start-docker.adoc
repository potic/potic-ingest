...
docker run -dit --name pocket_square_articles --link pocket_square_ingest --link pocker_square_mongo -e MONGO_PASSWORD=MONGO_PASSWORD -v /logs:/logs -p 28103:8080 pocketsquare/pocket-square-articles:$TAG
----
